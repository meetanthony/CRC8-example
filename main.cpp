#include <QCoreApplication>
#include <cstdint>
#include <vector>

// CRC8 table (replace with your own table)
const uint8_t CRC8_TABLE[256] = /*{
    

}*/;

// Function to reverse bits in a byte
uint8_t reverseBits(uint8_t byte) {
    uint8_t result = 0;
    for (int i = 0; i < 8; ++i) {
        result = (result << 1) | (byte & 0x01);
        byte >>= 1;
    }
    return result;
}

uint8_t calculateCRC8(const std::vector<uint8_t>& data,
                      uint8_t init,
                      bool refIn,
                      bool refOut,
                      uint8_t xorOut) {
    uint8_t crc = init; // Initial CRC value

    for (uint8_t byte : data) {
        if (refIn) {
            byte = reverseBits(byte); // Reverse input byte if required
        }
        crc = CRC8_TABLE[crc ^ byte];
    }

    if (refOut) {
        crc = reverseBits(crc); // Reverse result if required
    }

    crc ^= xorOut; // Apply XorOut

    return crc;
}

int main() {
    // Example data
    std::vector<uint8_t> data = {0x12, 0x34, 0x56, 0x78};

    // CRC8 parameters
    uint8_t init = ;//0xFF;   // Initial value
    bool refIn = ;//false;    // Reverse input bytes
    bool refOut = ;//false;   // Reverse output CRC
    uint8_t xorOut = ;//0xFF; // Final XOR value

    uint8_t crc = calculateCRC8(data, init, refIn, refOut, xorOut);

    printf("CRC8: 0x%02X\n", crc);

    return 0;
}
